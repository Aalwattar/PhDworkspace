...
//
	for (i=0;i<numberOfTasks;i++)
	{
		...
		TasksTable[i].Sim.ConfigTV.ExpNo=0;
		TasksTable[i].Sim.ConfigTV.Value=0;
...
void getTaskSimulation( int ID,struct Simulation *value)
{
	...
	value->ConfigTV.ExpNo=TasksTable[ID].Sim.ConfigTV.ExpNo;
	value->ConfigTV.Value=TasksTable[ID].Sim.ConfigTV.Value;
}
...
void setTaskSimulation (int ID,struct Simulation *value)
{
		...
		TasksTable[ID].Sim.ConfigTV.ExpNo=value->ConfigTV.ExpNo;
		TasksTable[ID].Sim.ConfigTV.Value=value->ConfigTV.Value;
}
